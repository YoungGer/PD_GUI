function [pd] = prpd_analysis(full_name, prpd_multi)
...
for k=1:3
    ...
    for i = 1:k
        ...
        if (length(pos_theta)<1 | length(neg_theta)<1) 
            ...
            display('!!!k=1');
            display(diff_angle) ;
            if (diff_angle>pi*(1.0-prpd_multi) & diff_angle<pi*(1.0+prpd_multi))  %放电判据
               display('!!!pd') ;
               pd = 1;
               return
            end
        ...
        if (length(pos_theta)<2 | length(neg_theta)<2) 
            ...
            %判据
            if (diff_angle1>pi*(1.0-prpd_multi) & diff_angle1<pi*(1.0+prpd_multi)) & ...
               (diff_angle2>pi*(1.0-prpd_multi) & diff_angle2<pi*(1.0+prpd_multi)) & ...
               ((neg_angle1>pi/3*(1.0-prpd_multi) & neg_angle1<pi/3*(1.0+prpd_multi)) | (neg_angle1>pi*2/3*(1.0-prpd_multi) & neg_angle1<pi*2/3*(1.0+prpd_multi)))   & ...
               ((neg_angle2>pi/3*(1.0-prpd_multi) & neg_angle2<pi/3*(1.0+prpd_multi)) | (neg_angle2>pi*2/3*(1.0-prpd_multi) & neg_angle2<pi*2/3*(1.0+prpd_multi)))
               display('!!!pd') 
               pd = 2;
               return;
            end
        ...
        if (length(pos_theta)<3 | length(neg_theta)<3) 
            ...
            if (diff_angle1>pi*(1.0-prpd_multi) & diff_angle1<pi*(1.0+prpd_multi)) & ...
               (diff_angle2>pi*(1.0-prpd_multi) & diff_angle2<pi*(1.0+prpd_multi)) & ...
               (diff_angle3>pi*(1.0-prpd_multi) & diff_angle3<pi*(1.0+prpd_multi)) & ...
               ((neg_angle1>pi/3*(1.0-prpd_multi) & neg_angle1<60*(1.0+prpd_multi)) | (neg_angle1>pi*2/3*(1.0-prpd_multi) & neg_angle1<pi*2/3*(1.0+prpd_multi)))   & ...
               ((neg_angle2>pi/3*(1.0-prpd_multi) & neg_angle2<60*(1.0+prpd_multi)) | (neg_angle2>pi*2/3*(1.0-prpd_multi) & neg_angle2<pi*2/3*(1.0+prpd_multi)))   & ...
               ((neg_angle3>pi/3*(1.0-prpd_multi) & neg_angle3<60*(1.0+prpd_multi)) | (neg_angle3>pi*2/3*(1.0-prpd_multi) & neg_angle3<pi*2/3*(1.0+prpd_multi)))   & ...
               ((neg_angle4>pi/3*(1.0-prpd_multi) & neg_angle4<60*(1.0+prpd_multi)) | (neg_angle4>pi*2/3*(1.0-prpd_multi) & neg_angle4<pi*2/3*(1.0+prpd_multi)))   
               display('!!!pd') 
               pd = 3;
               return
